...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:3.0\n' 
            + 'FN: Minato YT 😳\n' 
            + 'ORG: Criador Minato YT 😳;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=5511934488829:+55 55-93448-8829\n' 
            + 'END:VCARD'
prefix = '/'
blocked = []   
...
const pacarNumber = ["5511934488829@s.whatsapp.net"]
/************************************/
...
itsmeiky.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Roiii* 👋 @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\Espero que se sinta em casa aqui😅\nLeia As Regras Pra N Ser Removido @${num.split('@')[0]} 😅`
				...
				teks = `*Saudações Man😳* @${num.split('@')[0]}\n*I Sentirá Sua Falta* 🤭`
				let buff = await getBuffer(ppimg)
				itsmeiky.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	itsmeiky.on('message-new', async (iky) => {
		...
		if (isGroupAdmins) return reply('Como Você Er Admin Do Grupo Não Vou Remove, Mais Queria Tanto😔')
		itsmeiky.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#permissão de administrador recebida")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Não Pode Link Demonio🤬 ${sender.split("@")[0]} Desculpa Mais Vou Ter Que Remove`)
		setTimeout( () => {
		...
		case 'mutual': // Fix Bug By ItsmeikyXSec404				
				...
				await reply('Encontre um parceiro..')
		...
		case 'transfer':
				...
				if (!q.includes('|')) return  reply(ind.wrongf())
                		...
                		addKoinUser('5511934488829@s.whatsapp.net', fee)
              ...
              case 'Oi':
				...
				reply(`Administrador de bot adicionado com sucesso wa.me/${admm} `)
				break
                case 'unadmin':
				if (!isOwner) return reply(ind.ownerb())
				admm = body.slice(9)
				...
				fs.writeFileSync('./database/user/admin.json', JSON.stringify(adm))
				reply(`Administrador de bot adicionado com sucesso wa.me/${adm} `)
				break
                case 'wakillist':
				itsmeiky.updatePresence(from, Presence.composing) 
				 // Fix Bug By ItsmeikyXSec404				
				...
				reply(`Parabéns Você Viro Usuário Premium wa.me/${premm} `)
				break
		case 'unpremium':
				if (!isOwner) return reply(ind.ownerb())
				premm = body.slice(11)
				...
				break
                case 'bann':
				if (!isAdmin) return reply('*Apenas Admin Do BOT *')
				bnnd = body.slice(5)
				...
				break
		case 'unbann':
				if (!isAdmin) return reply('*Apenas Admin Do Bor*')
				bnnd = body.slice(7)
		...
		case 'unban':
				...
				itsmeiky.updatePresence(from, Presence.composing) 
				 // Fix Bug By ItsmeikyXSec404				
                 if (!isRegistered) return reply( ind.noregis())    
				teks = 'Esta é uma lista de números proibidos :\n'
				for (let benn of ban) {
					teks += `~> @${benn.split('@')[0]}\n`
					}
		...
		case 'lb':
				...
				break
					...
					if ((args[0]) === '1') {
						...
						reply('1 para ativa, 0 para desativa')
					}
					break
		case 'kalkulator':
				 // Fix Bug By ItsmeikyXSec404				
				...
				if (typeof Math_js.evaluate(mtk) !== "number") {
					reply(`"${mtk}", Erro!\n[❗] Enviar pedidos *${prefix}kalkulator [ Angka ]*\nContoh : ${prefix}kalkulator 12*12\n*NOTE* :\n• Para multiplicação usando *\n• Untuk Pertambahan Menggunakan +\n• Untuk Pengurangan Menggunakan -\n• Untuk Pembagian Menggunakan /`)
				} else {
		...
		case 'giftlimit': 
				 // Fix Bug By ItsmeikyXSec404				
                 if (!isRegistered) return reply( ind.noregis())
				if (!isAdmin) return reply('*Apenas nosso administrador e proprietário!*')
				const nomerr = args[0].replace('@','')
                		const jmla = args[1]
                		if (jmla <= 1) return reply(`o limite mínimo de presentes é 1`)
                		if (isNaN(jmla)) return reply(`limit harus berupa angka`)
                		if (!nomerr) return reply(`desculpe formato errado \ parâmetro correto inserido \ exemplo : ${prefix}giftlimit @5511934488829 20`)
		...
		case 'say':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        if (args.length < 1) return reply('Cadê O Texto Bixo Burro?')
                                        saying = teks
		...
		case 'info':
					...
					teks = `*Info Do Meu Criador *\n*Owner bot* : Minato YT 😳\n*No Owner* : wa.me/5511934488829\n*YouTube owner* : https://youtube.com/channel/UCWZKMTuAI8t4ENaCftJxqQw\n━━━━━━━━━━━━━━━━━━━━\n*INFO BOT*\n*Nama Minato* : ${me.name}\n*Numero Do Minato bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total block contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}\n*Fans page FB* : https://www.facebook.com/ainneboot\n*Ketik* : ${prefix}lpr _Untuk melaporkan admin bot melalui bot_\n*Ketik* : ${prefix}owner untuk menghubungi admin bot kami.`
		...
		case 'lpr':
					...
					itsmeiky.sendMessage('5511934488829@s.whatsapp.net', options, text, {quoted: iky})
					reply('Problemas foram relatados ao proprietário do MINATO BOT, relatórios falsos ou confusos não serão respondidos .')
					break
		case 'request':
					...
					itsmeiky.sendMessage('5511934488829@s.whatsapp.net', options, text, {quoted: iky})
					itsmeiky.sendMessage('5511934488829@s.whatsapp.net', options, text, {quoted: iky})
					reply('SUA SOLICITAÇÃO FOI ENTREGUE AO PROPRIETÁRIO Shizuka BOT, solicitações falsas ou rede elétrica não serão respondidas.')
					break
		case 'blocklist': 
		...
		case 'tts':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (!isPremium) return reply('Desculpe, Você Não Er Um Usuário Prwmium!')
				if (isBanned) return reply('Você Tá Banido Krlho')
				if (args.length < 1) return itsmeiky.sendMessage(from, 'Diperlukan kode bahasa kak!!', text, {quoted: iky})
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('O texto é principalmente demônio😤')
		...
		case 'addlimitt':
					if (args.length < 1) return
				if (!isAdmin) return reply('*Apenas Admin Ou Dono*')
					limitawal = args[0]
					reply(`*Limit berhasil di ubah menjadi* : ${limitawal}`)
					break 
		case 'setmemlimit':
		...
		case 'setpp': 
					...
					reply('*Sucesso Mudo O Ícone Do Grupo*')
					break				
		case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args.length < 1) return reply('Quem Desaja Adiciona?')
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar destino, provavelmente porque er privado')
					}
					break
		case 'grup':
		case 'group':
					...
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args[0] === 'buka') {
					    reply(`*GRUPO TÁ ABERTO PODEM FALA SEUS CORNO*`)
						itsmeiky.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`*GRUPO FECHADO ATÉ QUANDO UM ADMIN ABRIR PARCA*`)
		...
		case 'simih':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Para Ativa Digite 1 Para Desativa Digite 0')
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('*Já Está Ativo Seu Tonto*')
						samih.push(from)
						...
						samih.splice(from, 1)
						fs.writeFileSync('./database/bot/simi.json', JSON.stringify(samih))
						reply('Sucesso Desativado Modo Simi Neste Grup✔️')
					} else {
						reply(ind.satukos())
					}
					break
		case 'nsfw':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Para Ativa Digite 1 Para Desativar Digite 1')
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('*Já Está Ativo Tonto *')
						nsfw.push(from)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Sucesso Modo NSFW Está Ativo ✔️')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Sucesso Você Desativou Modo NSFW ✔️')
					} else {
						reply(ind.satukos())
					}
					break
		case 'leveling':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Digite 1 Para Ativa/Para Desligar 0')
					if (args[0] === '1') {
					if (isLevelingOn) return reply('*Já Está Ativo Tonto*')
					_leveling.push(from)
		...
		case 'welcome':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Digite 1 Para Ativa/0 Para Desativa')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('*Já Está Ativo Tonto ')
						welkom.push(from)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ Sucesso Você Ativou Modo Bem Vindo Neste Grupo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ Sucesso Você Desativou Modo B Vindo Neste Grupo')
					} else {
						reply(ind.satukos())
					}
					break
                 case 'antilink':
					...
					if (args.length < 1) return reply('1 Para Ativa/Para Desativa 0')
					if (Number(args[0]) === 1) {
						if (isAntilink) return reply('Já Está Ativo Tonto ')
						antilink.push(from)
						fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
						reply('Sucesso Modo Antilink Está Ativo ✔️')
						...
						fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
						reply('Grupo anti-link desativado com sucesso neste grupo ✔️✔️')
					} else {
                 ...
                 case 'event':
					if (!isGroup) return reply(ind.groupo())
					if (!isAdmin) return reply('Apenas Nosso Admin Ou Meu Dono')
					if (args.length < 1) return reply('Para Ativa 1, Para Desativa 0')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*Já Está Ativo Tonto*')
						event.push(from)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('❬ Sucesso ❭ Você Ativou O Recurso Evento Neste Grupo')
					} else if (Number(args[0]) === 0) {
						event.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('❬ Sucesso ❭ Você Desativou O Recurso Evento Neste Grupo')
					} else {
						reply(ind.satukos())
					}
					break
		...
		case 'clone':
					...
					if (args.length < 1) return reply(' *TAG Do Alvo Que Deseja Clona!!!* ')
